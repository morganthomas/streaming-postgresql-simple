...
let
  ...
  streaming-postgresql-simple-overlay = self: super: {
  ...
  pkgs = import
    ...
    overlays = [
      shpadoinkle-overlay
      streaming-postgresql-simple-overlay
    ];
  };


  # We can name him George
  streaming-postgresql-simple = pkgs.haskell.packages.${compilerjs}.callCabal2nix "streaming-postgresql-simple" (gitignore ignorance ./.) {};
...
in with pkgs; with lib; with haskell.packages.${compiler};
  ...
  then shellFor {
    inherit withHoogle;
    packages = _: [streaming-postgresql-simple];
    COMPILER = compilerjs;
    buildInputs = [ stylish-haskell cabal-install ghcid ];
    shellHook = ''
      ${lolcat}/bin/lolcat ${./figlet}
    '';
  } else chill streaming-postgresql-simple
